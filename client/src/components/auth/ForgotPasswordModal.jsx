      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[32px] sm:rounded-[36px] shadow-2xl border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8 overflow-hidden transition-all">
        ...
        {step === 1 && (
          <form onSubmit={handleInitiateOtp} className="space-y-4 relative z-10">
            ...
            {/* Active Account Identity Banner from Database */}
            {userInfo ? (
              <div className="flex items-center justify-between px-3.5 py-2 rounded-2xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200/80 dark:border-slate-700/60 text-xs">
                <div className="flex items-center gap-2">
                  <UserCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span className="font-semibold text-slate-700 dark:text-slate-300">
                    {userInfo.fullName} {userInfo.employeeCode ? `(${userInfo.employeeCode})` : ''}
                  </span>
                </div>
                <span
                ...
                >
                  {isHrOrAdmin ? 'HR Administrator' : 'Staff Employee'}
                </span>
              </div>
            ) : (
              !checkingUser && (
                <div className="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/40 text-[11px] text-slate-500 dark:text-slate-400 border border-slate-200/60 dark:border-slate-800">
                  Enter your registered work email. The system actively verifies and loads your linked WhatsApp number from the database.
                </div>
              )
            )}
            ...
            {/* Dynamic Real-Time Delivery Hint */}
            {isHrOrAdmin ? (
              <div className="p-3 rounded-2xl bg-blue-50/80 dark:bg-blue-950/40 border border-blue-200/80 dark:border-blue-800/60 flex items-start gap-2.5 text-xs text-blue-800 dark:text-blue-300">
                <ShieldAlert className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
                <div className="text-[11px] leading-relaxed">
                  <span className="font-bold">Enterprise Security Policy:</span> HR accounts are verified strictly through authenticated Corporate Email.
                </div>
              </div>
            ) : channel === 'WHATSAPP' ? (
